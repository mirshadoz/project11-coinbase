...
const drawPriceLegend = () => {
  ...
  for(let cryptoPrice=0; cryptoPrice<=cryptoPriceLegendMax; cryptoPrice+=15000) {
...
const drawPriceLineChart = () => {
  ...
  data.forEach(crypto => {
    ctx.lineTo(blocks(temp+xPlot), tempPosition.y - (Number(crypto.price)/200));
    ctx.arc(blocks(temp+xPlot), tempPosition.y - (Number(crypto.price)/200), 3, 0, Math.PI*2, true);
    ctx.strokeText(crypto.symbol, blocks(temp+xPlot), tempPosition.y - (Number(crypto.price)/200));
    xPlot += 8;
  });
