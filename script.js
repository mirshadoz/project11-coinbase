...
const get_datas = async () => {
  ...
  let response = await fetch(
    `${proxy_URL}${BASE_URL}/v1/cryptocurrency/listings/latest?start=1&limit=5`,
    {
      method: 'GET',
      headers: {
  ...
  cryptos.forEach((crypto) => {
    ...
    table_cryptos += `<td> ${Number(
      crypto.quote.USD.volume_24h.toExponential(),
    ).toPrecision(2)} </td>`;
    table_cryptos += `<td> ${crypto.quote.USD.percent_change_24h.toFixed(
      1,
    )} </td>`;
